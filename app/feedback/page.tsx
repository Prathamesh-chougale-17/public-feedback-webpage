...
<Select
      ...
      data={['Banipark police station', 'Bhakrota police station','Chitrakoot police station','Chomu police station','Sikar Rd police station','Kalwar Rd police station']}
      defaultValue="React"
      withAsterisk
...
const FeedbackForm = () => {
  ...
  const processForm: SubmitHandler<FormSubmittion> = async (data) => {
    ...
    if ( saveMessage === 500 || saveMessage == 401 || saveMessage == 400 || saveMessage == 404 || saveMessage == 403) {
    ...
    if (sendMessage === 500 || sendMessage == 401 || sendMessage == 400 || sendMessage == 404 || sendMessage == 403) {
      toast("Something went wrong!");
      return;
    }
  }
  ...
  const form = useForm({
    ...
    validate: {
      ...
      phoneNumber: (value) => (value.length === 10 ?  null:'Phone number must have 10 digits' ),
      gender:(value)=>(value.length<4 ? "Please select your Gender":null),
      // address: (value) => (value.length < 10 ? 'Address must have at least 10 letters' : null),
      feedback: (value) => (value.length < 10 ? 'Feedback must have at least 10 letters' : null),
      rating: (value) => (value < 1 ? 'Please give rating for the service provided' : null),
      satisfaction: (value) => (value.length < 1 ? 'Please choose an option' : null),
    }
  })
